...
TypedFocalSearch::TypedFocalSearch(const plugins::Options &opts)
...
void TypedFocalSearch::insert_typed(const State &s, int hf, bool is_pref) {
    auto &buckets = use_pref_list(is_pref) ? typed_pref : typed_focal;
    buckets[hf].push_back(s.get_id());
    h_focal_val[s]        = hf;
    generated_by_pref[s]  = is_pref;
    in_focal[s]           = true;
}

// Linear scan is fine: buckets are typically small (few states per h_focal value).
void TypedFocalSearch::remove_typed(const State &s) {
    int hf       = h_focal_val[s];
    bool is_pref = generated_by_pref[s];
    auto &buckets = use_pref_list(is_pref) ? typed_pref : typed_focal;

    auto it = buckets.find(hf);
...
// Pick the bucket with the lowest h_focal value and return a uniformly random
// state from it (swap-and-pop for O(1) removal).
StateID TypedFocalSearch::extract_typed(bool from_pref) {
    auto &buckets = from_pref ? typed_pref : typed_focal;
    assert(!buckets.empty());

    auto it    = buckets.begin();   // min h_focal
    auto &vec  = it->second;
    assert(!vec.empty());
...
void TypedFocalSearch::initialize() {
    const char *pb_str[] = {"typed_focal only", "alternating", "pref_first"};
    log << "Conducting Typed Focal Search (K=" << k << ")"
    ...
    insert_typed(initial_state, hf, false);
...
SearchStatus TypedFocalSearch::step() {
    ...
    for (int batch = 0; batch < k; ++batch) {
        ...
        if (id == StateID::no_state) break;  // FOCAL exhausted; stop batch early

        State s        = state_registry.lookup_state(id);
        SearchNode node = search_space.get_node(s);

        // Decrement count_f for this state leaving FOCAL
        count_f[f_value[s]]--;
        ...
        for (OperatorID op_id : applicable_ops) {
            ...
            if (succ_node.is_new()) {
                ...
                // Use open_eval to determine reachability (admissible dead-end check)
                ...
                if (fv <= w_f_min) {
                    insert_typed(succ_state, hf, is_pref);
                    count_f[fv]++;
                } else {
                ...
                // Shorter path found; reopen_closed = false → just update parent
    ...
    if (f_min > prev_f_min) {
        while (!open_list->empty()) {
            ...
            insert_typed(s_top, h_focal_val[s_top], generated_by_pref[s_top]);
...
void TypedFocalSearch::print_statistics() const {
    statistics.print_detailed_statistics();
    search_space.print_statistics();
}

void TypedFocalSearch::dump_search_space() const {
    search_space.dump(task_proxy);
}

void TypedFocalSearch::start_f_value_statistics(EvaluationContext &eval_context) {
    if (focal_evaluator) {
