...
"""
Experiment: Focal Search w=1 (cg open / ff focal) vs Eager Search (tiebreaking cg+ff)
Domain: logistics98
Date: 23-04-2026
"""
...
# ---------------------------------------------------------------------------
# focal_search w=1: inadmissible tiebreaking within the focal list is disabled
# (w=1 means the focal list only contains states with f <= 1 * f_min, i.e. optimal)
CONFIGS = [
    (
        "focal-w1-cg-ff",
        ["--search", "focal_search(open_eval=cg(), focal_eval=ff(), w=1)"],
    ),
    (
...
ALGO_PAIR = ("focal-w1-cg-ff", "eager-tiebreak-cg-ff")

project.add_absolute_report(
    exp,
    name="report-FSvsTiebreaking",
    attributes=ATTRIBUTES,
    filter=project.add_evaluations_per_time,
)

# Scatter plots comparing focal w=1 vs eager tiebreaking
for attribute in ["total_time", "expansions"]:
    exp.add_report(
        ...
        name=f"scatter-FSvsTiebreaking-{attribute}",
    )
